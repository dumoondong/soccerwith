...
<%
	...
	for(CommentTO to : comadatas) {
		...
		String recoid = to.getRecoId();
		String recopseq = to.getRecoPseq();
		
		sbHtml.append("<div class='input-group'>");
		sbHtml.append("<h6>"+id+"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+wdate+"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<i class='bx bxs-like'></i>"+recommend+"");
		sbHtml.append("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='/admin_comment_delete.do?id="+id+"&seq="+seq+"&pseq="+pseq+"'>삭제</a>");
		sbHtml.append("</h6>");
		...
		if(loginUser != null){
			if(loginUser.equals(recoid) && seq.equals(recopseq)){
				sbHtml.append("<a href='./admin_comment_cancel.do?id="+loginUser+"&seq="+seq+"&pseq="+pseq+"' <button style='width:70px; float: right;' type='button' class='btn btn-secondary btn-sm'><i class='bx bxs-like'></i></button></a>");
			}else{
				sbHtml.append("<a href='./admin_comment_Recommend.do?id="+loginUser+"&seq="+seq+"&pseq="+pseq+"' <button style='width:70px; float: right;' type='button' class='btn btn-primary btn-sm'><i class='bx bxs-like'></i></button></a>");
			}
		}
	...
	for(CommentTO to : likedatas) {
		...
		lkHtml.append("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='/admin_comment_delete.do?id="+id+"&seq="+seq+"&pseq="+pseq+"'>삭제</a>");
		lkHtml.append("</h6>");
		...
		if(loginUser != null){
			if(loginUser.equals(recoid) && seq.equals(recopseq)){
				lkHtml.append("<a href='./admin_comment_cancel.do?id="+loginUser+"&seq="+seq+"&pseq="+pseq+"' <button style='width:70px; float: right;' type='button' class='btn btn-secondary btn-sm'><i class='bx bxs-like'></i></button></a>");
			}else{
				lkHtml.append("<a href='./admin_comment_Recommend.do?id="+loginUser+"&seq="+seq+"&pseq="+pseq+"' <button style='width:70px; float: right;' type='button' class='btn btn-primary btn-sm'><i class='bx bxs-like'></i></button></a>");
			}
		}
...
/*  	ArrayList<RecommendTO> Joindatas = (ArrayList)request.getAttribute( "Joindatas" ); // 댓글 리스트
	...
	if(loginUser != null){	//로그인 한상태일때만 추천 가능
		...
		if(loginUser.equals(likeid) && likeseq.equals(seq)){
			reHtml.append("<a href='./admin_board_cancel.do?id="+loginUser+"&seq="+seq+"' <button style='width:70px;' type='button' class='btn btn-secondary btn-sm'><i class='bx bxs-like'></i></button></a>");
		}else{
			reHtml.append("<a href='./admin_board_Recommend.do?id="+loginUser+"&seq="+seq+"' <button style='width:70px;' type='button' class='btn btn-primary btn-sm'><i class='bx bxs-like'></i></button></a>");
		}
	}
%>
<!DOCTYPE html>
	...
	<div align="middle">
		...
		<%if(loginUser != null){ %>		 <!--로그인 했을경우  -->
			...
			<div class="btn-group" style="margin: 0px 0px 40px 1290px;" role="group" aria-label="Basic example">
				<button type="button" class="btn btn-secondary btn-sm" onclick="location.href='./admin_board_modify.do?seq=<%=seq %>&id=<%=id%>&smallcode=<%=smallcode %>'">수정</button>
				<button type="button" class="btn btn-secondary btn-sm" onclick="deleteok()">삭제</button>
				<button type="button" class="btn btn-secondary btn-sm" onclick="location.href='./admin_board.do'">목록</button>
			</div>			
		<%} %>
		<form action="admin_board_write_ok.do" method="post" name="mfrm" >
			<div class="wrapper" style="text-align:left; width:60%; border:1px solid black; padding: 30px 30px 30px 30px; " ><hr />
				<div class="input-group">
				   <h2><%=title %></h2>
				</div><hr />
				<h6>ID : <%=id %></h6>	
				<h6>작성일 : <%=wdate %>&nbsp;｜&nbsp;조회&nbsp;:&nbsp;<%=hit %>&nbsp;｜&nbsp;<i class='bx bxs-like'></i>&nbsp;:&nbsp;<%=recommend %>&nbsp;｜&nbsp; 
					<%if(loginUser != null){ %>
						<%=reHtml %>
					<%} %>
				</h6><hr />
				<div style="width:100; word-break:break-all;word-wrap:break-word;"><%=content %></div>
		</form><br /><br /><hr /><br />
		<%if(loginUser != null){ %>		 <!--로그인 했을경우  -->
			<input type="hidden" name="id" value="<%=loginUser %>" />
		...
		<%} %>
		<%if(selinfo != null && selinfo.equals("공지사항")){}else{ %>
			<h4>댓글 : <%=count %>개</h4>
			...
			<form action="admin_comment_write_ok.do" method="post" name="mfrm" >
				<div class="input-group">
					 <h4>댓글쓰기</h4>
				</div>
				<div class="input-group">
...
<script>
  ...
  function deleteok() {
      if (!confirm("삭제하시겠습니까?")) {
    	  ...
    	  location.href="admin_board_delete_ok.do?seq=<%=seq%>";    
      }
  }
</script>
</body>
