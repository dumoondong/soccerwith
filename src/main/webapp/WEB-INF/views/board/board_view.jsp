...
<%
	...
	for(CommentTO to : comadatas) {
		...
		if(loginUser != null){
			if(loginUser.equals(recoid) && seq.equals(recopseq)){
				sbHtml.append("<a href='./comment_cancel.do?id="+loginUser+"&seq="+seq+"&pseq="+pseq+"' <button style='width:70px; float: right;' type='button' class='btn btn-secondary btn-sm'><i class='bx bxs-like'></i></button></a>");
			}else{
	...
	for(CommentTO to : likedatas) {
		...
		if(loginUser != null){
			if(loginUser.equals(recoid) && seq.equals(recopseq)){
				lkHtml.append("<a href='./comment_cancel.do?id="+loginUser+"&seq="+seq+"&pseq="+pseq+"' <button style='width:70px; float: right;' type='button' class='btn btn-secondary btn-sm'><i class='bx bxs-like'></i></button></a>");
			}else{
...
<body>
	...
	<div align="middle">
		...
		<%if(loginUser != null){ %>		 <!--로그인 했을경우  -->
			...
			<%}else{%>					<!-- 로그인id와 작성id가 같지 않을경우 -->
				<button type="button" style="float: right;" class="btn btn-secondary btn-sm" onclick="location.href='./board.do'">목록</button><br /><br />
			<%}}else{%>					<!-- 로그인 안했을경우 -->
			<button type="button" style="float: right;" class="btn btn-secondary btn-sm" onclick="location.href='./board.do'">목록</button><br /><br />
		<%} %>
